...
```{math}r_{\perp} = \frac{\sin^2(\theta_1-\theta_2)}{\sin^2(\theta_1+ \theta_2)}
\label{eq:rperp}
```

e, para a polarização perpendicular ao plano de incidência:
\begin{equation}\label{eq:rparallel}
...
Observe também que as equações \eqref{eq:rperp} e \eqref{eq:rparallel} são simétricas se trocarmos a ordem dos meios. Isso implica que a interface  de forma igual em ambos os lados.
