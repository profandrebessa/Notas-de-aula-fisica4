...
Observe também que as equações {eq}`eq:rperp` e {eq}`eq:rparallel` são simétricas se trocarmos a ordem dos meios. Isso implica que a interface  de forma igual em ambos os lados.
